...
def generate_test_negatives(index_range, df_all, df_test, df_train, test_neg_num, num_items):
    ...
    while i < index_range[1]:
        ...
        for t in range(num_negatives):
            j = np.random.randint(num_items)
            while (len(df_all[(df_all['user_id'].isin([uid])) & (df_all['item_id'].isin([j]))]) > 0) \
                    or (len(df_train[(df_train['user_id'].isin([uid])) & (df_train['item_id'].isin([j]))]) > 0):
                j = np.random.randint(num_items)
            each_negatives.append(j)
        i+=1
        logger.info(i)
...
def generate_and_store_test_negatives(df_all, df_test, df_train, test_neg_num, test_pos_neg_path):
    ...
    num_items = len(df_all['item_id'].unique())
...
if __name__ == '__main__':
    ...
    path = '../datasets/ciao/ciao_inter.csv'
    df = pd.read_csv(path)
    train_data = df[df['x_label'] == 0][['user_id', 'item_id', 'rating']]
    test_data = df[df['x_label'] == 1][['user_id', 'item_id', 'rating']]
    print(len(test_data))
    generate_and_store_test_negatives(df, test_data, train_data, 99,
                                      '../datasets/ciao/ciao_test.txt')
